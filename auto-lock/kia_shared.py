...
class KiaSession:
  ...
  def has_valid_access_token(self) -> bool:
    return bool(self.access_token) and time.monotonic() < self.access_token_expires_at - 10

  def has_valid_control_token(self) -> bool:
    return bool(self.control_token) and time.monotonic() < self.control_token_expires_at - 10
...
class StampGenerator:
  ...
  def __call__(self) -> str:
    epoch_ms = int(datetime.datetime.now(datetime.UTC).timestamp() * 1000)
    payload = f"{self._app_id}:{epoch_ms}".encode()
...
class KiaBaseClient:
  ...
  def _ensure_vehicle(self) -> Dict[str, Any]:
    if not self.session.has_valid_access_token():
      self._login()
    elif self.creds.region in {Region.US, Region.EU, Region.AU, Region.CN} and time.monotonic() > self.session.access_token_expires_at - 120:
      self._refresh_access_token()
    if self._vehicle and time.monotonic() - self._vehicles_fetched_at < 300:
    ...
    self._vehicles_fetched_at = time.monotonic()
  ...
  def _login_us(self) -> None:
    ...
    self.session.access_token_expires_at = time.monotonic() + expires_in
  ...
  def _exchange_code_for_token_eu(self, code: str) -> None:
    ...
    self.session.access_token_expires_at = time.monotonic() + float(payload.get("expires_in", 1800))
  ...
  def _ensure_control_token_eu(self) -> None:
    ...
    self.session.control_token_expires_at = time.monotonic() + float(expires)
    logger.debug("EU control token acquired; expires in %s", expires)

  def _login_au(self) -> None:
    self.http.cookies.clear()
    session_url = (
      f"{self._env.base_url}/api/v1/user/oauth2/authorize?response_type=code"
      f"&client_id={self._env.client_id}"
      f"&redirect_uri={self._env.base_url}/api/v1/user/oauth2/redirect&lang=en"
    )
  ...
  def _init_cn_session(self) -> None:
    self.http.get(
      f"{self._env.base_url}/api/v1/user/oauth2/authorize?response_type=code"
      f"&client_id={self._env.client_id}"
      f"&redirect_uri={self._env.base_url}:443/api/v1/user/oauth2/redirect&state=test"
    )
