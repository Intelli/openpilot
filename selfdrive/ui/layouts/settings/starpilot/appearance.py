    ...
    def _build_view(self):
        ...
        self._model_rows = [
            ...
            SettingRow("EV9Path", "toggle", tr_noop("EV9 Path"),
                       subtitle=tr_noop("Blue while cruising, rainbow while accelerating. Warning highlighting is controlled separately."),
                       get_state=lambda: self._params.get_bool("EV9Path"),
                       set_state=lambda s: self._params.put_bool("EV9Path", s),
                       visible=model_on),
            SettingRow("PathWarningHighlight", "toggle", tr_noop("Path Warning Highlighting"),
                       subtitle=tr_noop("Highlight the path red for selected collision, steering-limit and driver-attention warnings."),
                       get_state=lambda: self._params.get_bool("PathWarningHighlight"),
                       set_state=lambda s: self._params.put_bool("PathWarningHighlight", s),
                       visible=model_on),
