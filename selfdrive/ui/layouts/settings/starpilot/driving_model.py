...
MANAGEMENT_STRIP_HEIGHT = 88.0
MANAGEMENT_PILL_HEIGHT = 72.0
...
class DrivingModelManagerView(AetherInteractiveMixin, Widget):
  ...
  def _draw_sort_strip(self, x: float, y: float, width: float, randomizer_on: bool):
    ...
    for i, mode in enumerate(_SORT_PILLS):
      ...
      draw_action_pill(seg_rect, label, fill, border, AetherListColors.HEADER, font_size=36, roundness=0.3)
  ...
  def _draw_download_action(self, rect: rl.Rectangle):
    ...
    gui_label(
      rl.Rectangle(rect.x + 16, rect.y + 72, rect.width - 32, 44),
      tr("Download"),
      36,
  ...
  def _draw_downloading_action(self, rect: rl.Rectangle, progress_text: str):
    ...
    gui_label(
      rl.Rectangle(rect.x + 12, rect.y + 72, rect.width - 24, 44),
      label,
      36,
      AetherListColors.SUBTEXT,
      FontWeight.MEDIUM,
  ...
  def _draw_menu_action(self, rect: rl.Rectangle, is_open: bool, entry: ModelCatalogEntry):
    if not is_open:
      ...
      draw_action_pill(fav_rect, fav_label, fav_fill, fav_border, fav_text_color, font_size=36)
