...
class AutoLockMonitor:
  ...
  def _get_kia_lock_module(self):
    if self._kia_lock_module is None:
      module_path = Path(__file__).resolve().parents[2] / "auto_lock" / "kia_lock.py"
  ...
  def _get_kia_status_module(self):
    if self._kia_status_module is None:
      module_path = Path(__file__).resolve().parents[2] / "auto_lock" / "kia_status.py"
      module_dir = module_path.parent
      if str(module_dir) not in sys.path:
        sys.path.insert(0, str(module_dir))
  ...
  def _get_kia_shared_module(self):
    if self._kia_shared_module is None:
      module_path = Path(__file__).resolve().parents[2] / "auto_lock" / "kia_shared.py"
      module_dir = module_path.parent
      if str(module_dir) not in sys.path:
        sys.path.insert(0, str(module_dir))
...
def main() -> None:
  ...
  if USE_CUSTOM_LOGFILE:
    log_dir = Path("/data/openpilot/auto_lock")
