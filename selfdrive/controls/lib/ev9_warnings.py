...
EV9_TRACKING_WARNING_ANGLE_DEG = 119.9
EV9_TRACKING_WARNING_CLEAR_ANGLE_DEG = 114.9
...
class EV9SteeringWarning:
  ...
  def update(self, *, now, active, speed, requested, measured, output, threshold_kph,
    ...
    if not all(math.isfinite(value) for value in (now, speed, requested, measured, dt)) or dt < 0 or dt > 0.2:
      self.elapsed, self.ready_elapsed, self.warning, self.tracking_warning = 0.0, 0.0, False, False
    ...
    tracking_threshold = EV9_TRACKING_WARNING_CLEAR_ANGLE_DEG if self.tracking_warning else EV9_TRACKING_WARNING_ANGLE_DEG
    tracking_angle = abs(requested) >= tracking_threshold
    ...
    gap_threshold = EV9_WARNING_CLEAR_GAP_DEG if self.tracking_warning else EV9_WARNING_TRACKING_GAP_DEG
    ...
    self.elapsed = self.elapsed + dt if tracking_angle and insufficient else 0.0
    ...
    self.tracking_warning = not recent_steer_pressed and tracking_angle and insufficient and (
      self.tracking_warning or self.elapsed >= EV9_WARNING_PERSISTENCE_SECONDS - 1e-6)
    self.warning = not recent_steer_pressed and ((eligible and controller_ready) or self.tracking_warning)
    return self.warning
