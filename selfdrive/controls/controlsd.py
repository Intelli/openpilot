...
class Controls(ControlsExt):
  ...
  def state_control(self):
    ...
    if self.sm.valid['lateralManeuverPlan']:
      base_desired_curvature = self.sm['lateralManeuverPlan'].desiredCurvature if CC.latActive else self.curvature
    else:
  ...
  @staticmethod
  def _history_average(history: deque) -> float | None:
  ...
  @staticmethod
  def _history_valid_fraction(history: deque) -> float:
    total = len(history)
    if total == 0:
      return 0.0
  ...
  def _update_deadband_thresholds(self) -> None:
    ...
    center_band = max(CENTERING_CENTER_BAND_MIN_M, min(CENTERING_CENTER_BAND_MAX_M, max(center_band, CENTERING_CENTER_BAND_BASE_M)))
  ...
  def _update_bounce_detection(self, active: bool, offset: float) -> None:
    ...
    while self._bounce_crossings and now - self._bounce_crossings[0][0] > CENTERING_BOUNCE_WINDOW_S:
  ...
  @staticmethod
  def _lane_y_at_distance(lane_line, distance_m: float) -> float | None:
    xs = lane_line.x
    ys = lane_line.y
  ...
  @staticmethod
  def _edge_clearance_target(left_edge: float | None, right_edge: float | None) -> tuple[float, bool]:
    left_violation = 0.0
    right_violation = 0.0
  ...
  @staticmethod
  def _edge_std_average(stds) -> float | None:
    if len(stds) == 0:
      return None
    total = 0.0
    count = 0
  ...
  @staticmethod
  def _offset_to_curvature(offset_m: float, min_offset: float, base_curvature: float, v_ego: float) -> float:
    ...
    if speed_kph > CENTERING_REFERENCE_SPEED_KPH:
      curvature_delta *= CENTERING_REFERENCE_SPEED_KPH / speed_kph
